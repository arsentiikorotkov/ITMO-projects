...
import com.web.multipart.MultipartTool
...
class MyService : Service() {
    ...
    var listOfMessages: List<MessageEntity> = mutableListOf()
    ...
    fun sendMessage(data: String, isItText: Boolean, onFinished: () -> Unit) {
        Thread {
            try {
                ...
                if (isItText) {
                    ...
                    outputStream.write("{\"from\":\"arsentiikorotkov\",\"data\":{\"Text\":{\"text\":\"$data\"}}}".toByteArray())
                } else {
                    ...
                    try {
                        tool.appendJsonField("msg", "{\"from\":\"arsentiikorotkov\"}")
                        tool.appendFile("pic", File(data))
                    } finally {
                        tool.close()
                        outputStream.close()
                    }
                }
                Log.i("responseCode", httpUrlConnection.responseCode.toString())
                httpUrlConnection.disconnect()
                onFinished()
            } catch (e: IOException) {
                e.printStackTrace()
