...
int main() {
    ...
    for (int i = 0; i < data.size(); i++) {
        ...
        for (int j = 0; j < edges[v - 1].size(); j++) {
            if ((edges[v - 1][j].to == u) && (edges[v - 1][j].id == id) && (edges[v - 1][j].f > 0)) {
                is_positive = true;
                cout << edges[v - 1][j].f << endl;
            }
        }
        
        for (int j = 0; j < edges[u - 1].size(); j++) {
            if ((edges[u - 1][j].to == v) && (edges[u - 1][j].id == id) && (edges[u - 1][j].f > 0)) {
                is_negative = true;
                cout << -edges[u - 1][j].f << endl;
            }
        }
