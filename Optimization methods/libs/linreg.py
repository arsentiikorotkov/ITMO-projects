...
def visualise_approximation(linreg, title):
    W = linreg.W.detach().numpy() if isinstance(linreg.W, torch.Tensor) else linreg.W
    x = np.linspace(min(linreg.X), max(linreg.X), 1000)
    y = sum([W[i] * linreg.T_funcs[i](x) for i in range(len(linreg.W))])
